...
- 既存サンプル: `samples/01_*` 〜 `samples/57_*` を踏まえた上での追加候補
...
**20〜57 は実装済み**（2026-05 時点）。A7 の f-string 詳細は、細かい仕様説明になりやすいため現時点では採用を見送った。
...
| 49 | 設定の扱い | 実装済み | `os.environ`、`tomllib`、dataclass による設定オブジェクト化を扱える |
| 50 | ファイル操作 | 実装済み | `pathlib` に加え、`shutil` / `tempfile` / `os.walk` の実務パターンを確認できる |
| 51 | 正規表現 `re` | 実装済み | ログ処理・文字列抽出・入力検証で頻出。raw string も説明できる |
| 52 | `sqlite3` | 実装済み | 標準ライブラリだけで DB 操作と SQL injection 回避の入口を示せる |
| 53 | DI を「軽く」やる | 実装済み | Java / Spring 経験者向けに、Python ではまず引数で依存を渡す発想を示せる |
| 54 | plugin / registry パターン | 実装済み | decorator の実務応用として handler 登録パターンを見せられる |
| 55 | `ruff` / `black` / `mypy` / `pyright` | 実装済み | ツール自体は導入せず、formatter / linter / type checker の役割を整理できる |
| 56 | `uv` の基本操作 | 実装済み | Python 本体、venv、依存、lock、実行をまとめて扱う現代的な入口になる |
| 57 | import 元の見分け方 | 実装済み | 標準ライブラリ / 3rd party / 自前コードを import 文だけで判別しにくい Python の事情を整理できる |
